    ...
    <div className="max-w-4xl mx-auto space-y-6">
          ...
          {unreadCount > 0 && (
            <>
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-purple-50 text-purple-700 hover:bg-purple-100 rounded-xl font-medium text-sm transition-all"
              >
                <CheckCheck className="w-4 h-4" />
                Mark all as read
              </button>
              <div className="px-4 py-2 bg-purple-100 text-purple-700 rounded-xl font-medium text-sm">
                {unreadCount} Unread
              </div>
            </>
          )}
      ...
      {loading ? (
        <div className="bg-white rounded-2xl p-12 border border-gray-200 text-center">
          <div className="animate-spin rounded-full h-8 w-8 border-4 border-slate-200 border-t-purple-600 mx-auto"></div>
          ...
          {notifications.map((notification) => (
            <div
              ...
              className={`p-5 hover:bg-gray-50/80 transition-colors cursor-pointer relative group ${
                notification.is_read === 0 ? "bg-purple-50/20" : ""
              }`}
            >
                  ...
                  <div className="flex items-start justify-between gap-4 mb-1">
                    ...
                    <div className="flex items-center gap-2">
                      {notification.is_read === 0 && (
                        <div className="w-2.5 h-2.5 bg-purple-600 rounded-full flex-shrink-0"></div>
                      )}
                      <button
