...
export default function DocumentViewer({
  ...
  const handleDownload = async (e: React.MouseEvent) => {
    e.preventDefault();
    try {
      const response = await fetch(getFileUrl(current.url));
      ...
      console.error("Failed to download file programmatically", err);
      window.open(getFileUrl(current.url), "_blank");
    }
  };
  ...
  const header = (
    <div className={`flex items-center justify-between px-8 py-5 border-b border-white/20 ${isFullscreen ? "bg-slate-900/90 backdrop-blur" : "bg-white/40"}`}>
      <div className="flex items-center gap-4">
        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg bg-gradient-to-br from-blue-500 to-indigo-600`}>
  ...
  const footer = attachments.length > 1 && (
  ...
  return (
    <div 
      className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 md:p-6 animate-fadeIn"
      onClick={onClose}
    >
      <div 
        className="w-full max-w-6xl h-[90vh] bg-white/80 backdrop-blur-xl rounded-[40px] shadow-2xl overflow-hidden flex flex-col border border-white/20 animate-scaleIn"
